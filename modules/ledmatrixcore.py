...
# TODO: should perhaps be merged with LEDMatrixCore class so as to be more in line with other modules
#  (self contained and inheriting from habase*)
class RGBMatrix(object):
    ...
    def get_json_state(self):
        ...
        if self.current_audio_thread is not None and self.current_audio_thread.isAlive():
        ...
        return json.dumps({'LEDMatrixCore': {'state': self.state, 'audiostate': audiostate,
                                             'audiostate_paused': audiostate_paused}})
    ...
    def on_enter_audiovisualizing(self, filename):
        ...
        try:
            ...
            pass
        lma = LEDMatrixAudio(name='Audio', rgbmatrix=self.rgbmatrix, sharedqueue=self.sharedqueue,
                             callback_function=self.to_idle, filepath=filename)  # which one is it?
        lma.start()
    ...
    def on_enter_settingmatrixfromimage(self, imagedata):
        ...
        lmi = LEDMatrixImage(name='Image', rgbmatrix=self.rgbmatrix, callback_function=self.to_idle,
...
class LEDMatrixCore(HomeAutomationQueueThread):
    ...
    def __init__(self, name, callback_function, queue, threadlist):
        """
        This class puts it all together, creating a state machine and a socket thread that calls
         state changes for received commands
        """
        ...
        self.machine = Machine(model=self.rgbm, states=self.states, transitions=self.transitions, initial='idle')

        lmssu = LEDMatrixSocketServiceUDP(name='SocketService', callback_function=None, rgbmatrix=self.rgbmatrix,
                                          rgbm=self.rgbm, queue=queue)
        # lmssu.start()
    ...
    def pre_processqueue(self):
        ...
        webservice_class_instances_add('ledmatrixaudio', self)
    ...
    def exec_item(self, item):
        try:
            # dont process more items from the queue unless the state machine is back to idle
            if self.rgbm.is_idle():
                # while this might look strange its the proper way of overriding methods
                super(LEDMatrixCore, self).exec_item(item)
        except MachineError:
